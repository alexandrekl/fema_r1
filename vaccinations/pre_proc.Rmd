...
```
Plot same as above but the the last 2 weeks
```{r echo=FALSE}
...
fcastfn <- function( l, c, al, be ){
  ...
  fdays <- as.Date('2021-10-01') - latest_history_date # number of days to forecast
...
```{r echo=FALSE}
print( paste('Latest data from', latest_history_date) )
tmp <- lapply( NEstates, fcastfn, 'total_distributed', NULL, NULL ) # 0.2, 0.05
f[[1]] <- bind_rows( tmp )
```
Forecast Cumulative number of doses **administered**
```{r echo=FALSE}
print( paste('Latest data from', latest_history_date) )
tmp <- lapply( NEstates, fcastfn, 'total_vaccinations', NULL, NULL ) # 0.07
...
tmp <- lapply( NEstates, fcastfn, 'daily_vaccinations', NULL, NULL ) # 0.07
f[[3]] <- bind_rows( tmp ) %>% select(-date,-location)
```
Forecast people vaccinated per capita
```{r echo=FALSE}
print( paste('Latest data from', latest_history_date) )
tmp <- lapply( NEstates, fcastfn, 'people_vaccinated_per_hundred', NULL, NULL ) # 0.07
